...
let solve' game =
	...
	Array.sort (fun i j -> compare (game#get_priority i) (game#get_priority j)) prio_arr;

	game#iterate (fun i -> fun (_,_,d,_,_) -> let delta = Array.of_list (ns_nodes d) in
						solver#add_helper_exactlyone 0 (Array.length delta - 1) [||] (fun j -> Po (Strategy (i, delta.(j)))));

	game#iterate (fun i -> fun (_,pl,delta,_,_) ->
			     ns_iter (fun j ->
	...
	plr_iterate (fun p -> 
	  game#iterate (fun i -> fun _ ->
            game#iterate (fun j -> fun _ ->
	...
	game#iterate (fun i -> fun (pr,_,_,_,_) -> solver#add_clause_array [|Ne (ReachLower (plr_opponent (plr_benefits pr), i, i, i))|]);
