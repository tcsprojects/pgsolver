...
let solve_single_player_scc game player =
  ...
  let complete_solution_and_strategy _ =
    ...
    let queue = SingleOccQueue.create () in
    for v=0 to n-1 do
      if solution.(v) != plr_undef then (
	ns_iter (fun w -> if solution.(w) = plr_undef then SingleOccQueue.add (w,v) queue) (pg_get_predecessors game v)) 
    done;
    while not (SingleOccQueue.is_empty queue) do
      ...
      ns_iter (fun u -> if solution.(u) = plr_undef then SingleOccQueue.add (u,w) queue) (pg_get_predecessors game w)
    done
  in
