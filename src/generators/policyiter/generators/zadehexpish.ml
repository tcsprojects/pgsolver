...
|	CycleNodeZ1 i    -> "e" ^ string_of_int i
|	CycleNodeZ2 i    -> "f" ^ string_of_int i
...
|	CycleNodeZ1T i    -> "h" ^ string_of_int i
|	CycleNodeZ2T i    -> "i" ^ string_of_int i
...
(*	add1  (PairSelector n) (2 * n + 11) [FinalSink];*)
	...
	for i = 0 to n - 1 do
		...
		add0  (CycleNodeZ1 i)  3  	 		                  [CycleCenter1 i; CycleNodeY1T i];
		add0  (CycleNodeZ1T i)  3  	 		                  [PairSelector 0; Selector 1];
		if i < n-1 then add0  (CycleNodeZ2 i)  3	 		    [CycleCenter2 i; CycleNodeZ2T i];
		if i < n-1 then add0  (CycleNodeZ2T i)  3	 		    [PairSelector 0; Selector 1];
		...
		add1  (CycleCenter1 i) 6  			  [CycleNodeX1 i; CycleNodeY1 i; CycleNodeZ1 i; UpDown1 i];
		if i < n-1 then add1  (CycleCenter2 i) 4  	 		  [CycleNodeX2 i; CycleNodeY2 i; CycleNodeZ2 i; UpDown2 i];
...
let register _ =
    register_strat_impr_gen {
        ident = "zadehexpish";
        description = "Exponentialish Lower Bound for Zadeh's rule";
