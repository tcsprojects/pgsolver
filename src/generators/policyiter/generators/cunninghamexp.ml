...
let generator_game_func arguments =
	...
	addnode (PacerEntry 0) 8 1 [if auso_compat then AusoHelper 0 else PacerNode (n-1)];
	addnode (PacerEntry 1) 3 1 [PacerEntry 0];
   
	for i = 0 to n - 1 do
        if auso_compat then addnode (AusoHelper i) 3 1 [if i = n-1 then PacerNode (n-1) else AusoHelper (i+1)];
...
let generator_mdp_func arguments =
	let game = generator_game_func arguments in
        parity_game_to_generalized_mdp game 8 (fun _ j -> pg_get_priority game (nd_make j) >= 8);;
