...
let generator_game_func arguments = 
  ...
  for i=0 to d-1 do
    ...
    for j=0 to w-1 do
      let j' = 2*j in
      print_node (name i' j') (m-i'-1) plr_Even [name (i'+1) (j'+1)] None
    done
  done;
  for i=0 to d-1 do
    let i'=2*i+1 in
    print_node (name i' 0) (m-i'-1) plr_Odd [name i' 1; name (i'-1) 0] None;
    for j=1 to w-1 do
      let j' = 2*j in
      print_node (name i' j') (m-i'-1) plr_Odd [name i' (j'-1); name i' (j'+1); name (i'-1) j'] None
    done;
    for j=0 to w-2 do
      let j' = 2*j+1 in
      print_node (name i' j') (m-i'-1) plr_Even [name i' (j'-1); name i' (j'+1); name (2*d) j'] None
    done;
    print_node (name i' (2*w-1)) (m-i'-1) plr_Even [name i' (2*w-1); name i' (2*w-2)] None
  done;

  print_node (name (2*d) 0) 0 plr_Even [name (2*d) 1] None;
  for j=1 to w-1 do
    let j' = 2*j in
    print_node (name (2*d) j') 0 plr_Even [name (2*d) (j'-1); name (2*d) (j'+1)] None
  done;
  for j=0 to w-2 do
    let j' = 2*j+1 in
    print_node (name (2*d) j') 1 plr_Odd ((name (2*d) (j'-1))::(name (2*d) (j'+1))::
                                            (List.map (fun i -> name (2*i+1) j') (range 0 (d-1)))) None
  done;
  print_node (name (2*d) (2*w-1)) 1 plr_Odd ((name (2*d) (2*w-2))::(name (2*d) (2*w-1))::
